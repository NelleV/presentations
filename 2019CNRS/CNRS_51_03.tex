...
\begin{itemize}
    ...
    données longitudinales d'expression génique}
\end{itemize}
\end{itemize}
...
\em Structure 3D du {\em P. falciparum}}
\begin{itemize}[leftmargin=*]
\tiny
\item[-]{%
\only<1-4>{%
...
\vspace{2em}
\textbf{EPICON \quad Étude de la dynamique de la réponse transcriptomique du sorgho sous stress de
sécheresse.}
\vspace{2em}
...
\item[-] 2 génotypes (l'un résistant à la sécheresse pré-floraison, l'autre
résistant à la sécheresse post-floraison)
...
\only<2->{\includegraphics[width=0.6\linewidth]{schemas/data_analysis_pipeline.pdf}}
\end{center}}
\end{overlayarea}
\begin{flushright}
...
\only<3->{%
\begin{itemize}
\item[-] Analyse d'expression différentielle fonctionnelle avec contrast
(inspiré d'EDGE)
\item[-] Clustering fonctionnel avec k-means
\item[-] Interpretation des clusters grâce à une analyse de terme GO et de
``pathways''
\end{itemize}}
\end{overlayarea}
\begin{flushright}
...
{\bf \small Quelques découvertes…}
\vskip-1.3ex
\rule{\dimexpr\linewidth-.5cm\relax}{0.4pt}
\begin{itemize}
\footnotesize
\item[-] Les processus de résistance au stress abiotique sont sur-exprimés,
ceux de résistance au stress biotique sous-exprimés.
\item[-] Les plantes essayent de maintenir les fonctions biologiques
(photosynthèse) des
feuilles le plus longtemps possible.
\item[-] Différence de détoxification des dérivés réactifs de l'oxygène entre
les deux génotypes, permettant à l'un génotype de maintenir la photosynthèse
plus longtemps.
\end{itemize}
\end{column}
\begin{column}{0.3\linewidth}
\centering
\includegraphics[width=\linewidth]{figures_epicon/fig2A_roots_versus_leaf_fr.pdf}
\end{column}
\end{columns}
\begin{flushright}
{\tiny {\color{red} Varoquaux$^*$} et al, en revue, Genome Research}
...
l'intégration de données ``-omics'' :}
...
\only<5>{%
\includegraphics[width=0.85\linewidth]{schemas/analyse_en_mediation_final.pdf}}
\end{figure}
\end{overlayarea}
\vspace{-1em}
\begin{overlayarea}{12cm}{5cm}
...
y_{\text{\tiny proteine}} &= f(X_{\text{\tiny mRNA}},
X_{\text{\tiny{miRNA}}}, X_{\text{\tiny treatment}}) \\
X^{\text{\tiny mRNA}} &=  f(X_{\text{\tiny methyl}}, X_\text{\tiny
3D}, X_{\text{\tiny TF}}) \\
\end{align*}}
\only<4->{%
\begin{align*}
...
\item[-] Inférence paramétrique, semi-paramétrique, ou non-paramétrique de la forme des fonctions.
\end{itemize}}
\end{column}
\end{columns}
\end{overlayarea}
...
\begin{column}{0.49\linewidth}
\begin{itemize}
\item[-] {\bf \color{red} Équipe ``Quantitative Regulatory Biology'' (Mirko
Francesconi)}
\item[-] Daniel Jost 
\item[-] 
\end{itemize}
