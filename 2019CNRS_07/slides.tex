...
\only<2>{
{\bf \small Les données épigénétiques : une vue multiple sur la cellule}
\vskip-1.3ex
\rule{\dimexpr\paperwidth-1.5cm\relax}{0.4pt}
\begin{itemize}
\small
...
\item[-]
\begin{equation*}
c_{ij} \sim \text{Poisson}(\beta_{ij} d_{ij}^\alpha)
\end{equation*}
...
\item[-]
\begin{equation*}
c_{ij} \sim \text{NB}(\beta_{ij} d_{ij}^\alpha, \delta_{ij})
\end{equation*}
\item[-] Maximization de la vraisemblance
\end{itemize}
}
...
\begin{frame}
\frametitle{Exemple II\quad Clustering de données d'expression génique
temporelles}
...
\begin{equation*}\label{eq:model} 
    \mathbb{E} c^k_{ij} = \beta^k_i \beta^k_j (1+T^k_{ij}) f^k(s(i,j)) 
\end{equation*}
