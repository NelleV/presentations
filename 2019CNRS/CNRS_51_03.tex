...
{\bf \small Découvertes principales}
\vskip-1.3ex
\rule{\dimexpr\linewidth-.5cm\relax}{0.4pt}
\begin{itemize}
\footnotesize
\item[-] Les plantes maintiennent le plus possible l'activité biologique des
feuilles.
\item[-] Les processus de résistance au stress biotique sont sous-exprimés.
\item[-] ROS scavenging differences in the genotype.
\item[-] La symbiose avec les AMF est interrompue.
\end{itemize}
\end{column}
\begin{column}{0.5\linewidth}
\centering
\includegraphics[width=0.7\linewidth]{figures_epicon/fig2A_roots_versus_leaf_fr.pdf}
\end{column}
\end{columns}
\begin{flushright}
{\tiny {\color{red} Varoquaux$^*$} et al, en revue, Genome Research}
...
\centering
\only<1>{%
\includegraphics[width=0.9\linewidth]{schemas/donnees_epigenetiques.pdf}}
\only<2->{%
\includegraphics[width=0.9\linewidth]{schemas/analyse_en_mediation_final.pdf}}
\end{figure}
\end{overlayarea}
