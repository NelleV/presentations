...
\begin{itemize}
    \scriptsize
    \item[-] {\bf Structure tri-dimensionnelle du génome}
\end{itemize}
...
\vspace{1em}
\only<2->{{\bf Problématique~:} Interpretation biologique entravée par le nombre de
gènes différentiellement exprimés.}

\vspace{1em}
\only<3->{{\bf But~:}\quad Développer une méthode de clustering prenant en
compte l'aspect temporelle des données afin de grouper les gènes possédant le
...
\begin{frame}
\frametitle{Contribution \quad Clustering de données temporelles d'expression génique}
\vspace{-2em}
\only<1>{%
\begin{figure}
...
\vspace{1em}
\only<2->{{\bf Défi~:} \quad Analyser et intégrer des données hétérogènes,
hétéroscedastiques, larges et difficile à manipuler, en haute dimension.}
...
\item[-] Programme de recherche: Apprentissage statistique en haute dimension : structures,
